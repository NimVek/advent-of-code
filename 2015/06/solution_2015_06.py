...
def parse(string):
    for command in string.splitlines():
        match = re.match(
            r"(?P<command>turn\s+(?P<switch>on|off)|toggle)\s+(?P<start_x>[0-9]+),(?P<start_y>[0-9]+)\s+through\s+(?P<end_x>[0-9]+),(?P<end_y>[0-9]+)",
            command,
        )
        if match:
            yield {
                "command": match.group("switch") or match.group("command"),
